...
create table if not exists public.daily_rosters_daily_derby (
  ...
  morning_line_odds text not null,
  condition_status text default 'Fresh',
  wildcard_trait text,
...
-- 4. Seed Today's Daily Race Roster (10 Horses with Odds & Wildcards)
...
select 
  (now() at time zone 'utc' - interval '8 hours')::date, -- Updated here
  id,
  case name
...
-- Add last_raced_date column to the horses table
