...
const WIDTH = Math.floor(document.getElementById("heading").offsetWidth)
const GAME_WIDTH = Math.floor(WIDTH/3);
const GAME_HEIGHT = 300;
const HEIGHT = GAME_HEIGHT*4;
...
class Game {
    constructor(id, x, y) {
        ...
        this.texts.score.position.set(this.x + scoreboard_x + 80, this.y + scoreboard_y);
        content.addChild(this.texts.score);

        this.texts.lbl_gen = new PIXI.Text("Generation:", {
            fontFamily: "Verdana", fontSize: 12, fill:0xffffff, align: "left",
        });
        this.texts.lbl_gen.anchor.set(0,0.5);
        this.texts.lbl_gen.position.set(this.x + scoreboard_x, this.y + scoreboard_y+14);
        ...
        this.texts.gen.position.set(this.x + scoreboard_x + 80, this.y + scoreboard_y+14);
        content.addChild(this.texts.gen);

        this.texts.lbl_name = new PIXI.Text("Name:", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "left",
        });
        this.texts.lbl_name.anchor.set(0, 0.5);
        this.texts.lbl_name.position.set(this.x + scoreboard_x, this.y + scoreboard_y+28);
        ...
        this.texts.name.position.set(this.x + scoreboard_x + 80, this.y + scoreboard_y+28);
    ...
    Update(dT) {
        ...
        // Input 4 = Bird speed.
        inputs.push(this.bird_speed/10);
        ...
        tree_bounds.right = this.imgs.tree.x + this.imgs.tree.width/2 - this.tree_speed*dT/2;
    ...
    Draw() {
        ...
        // DEBUG: draw arrow representing outputs.
        // Vertical line for output display.
        // let h_pos = this.x + 15;
        // graphics.moveTo(h_pos, this.y + LINE_HEIGHT - 40);
        // graphics.lineTo(h_pos, this.y + LINE_HEIGHT - 80);

        // // Arrow for up/down actions.
        // if (this.action_up) {
        //     graphics.moveTo(h_pos, this.y + LINE_HEIGHT - 80);
        //     graphics.lineTo(h_pos - 10, this.y + LINE_HEIGHT - 70);
        //     graphics.moveTo(h_pos, this.y + LINE_HEIGHT - 80);
        //     graphics.lineTo(h_pos + 10, this.y + LINE_HEIGHT - 70);
        // }
        // if (this.action_down) {
        //     graphics.moveTo(h_pos, this.y + LINE_HEIGHT - 40);
        //     graphics.lineTo(h_pos - 10, this.y + LINE_HEIGHT - 50);
        //     graphics.moveTo(h_pos, this.y + LINE_HEIGHT - 40);
        //     graphics.lineTo(h_pos + 10, this.y + LINE_HEIGHT - 50);
        // }
...
class ScoreOverGenPlot {
    constructor(x, y, w, h) {
        ...
        this.texts.title = new PIXI.Text("Top Score by Generation", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "center",
        });
        this.texts.title.anchor.set(0.5);
        this.texts.title.position.set(this.x + this.width/2, this.y + 15);
        content.addChild(this.texts.title);

        // X Labels.
        this.texts.xlabel = new PIXI.Text("Generation", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "center",
        });
        this.texts.xlabel.anchor.set(0.5,0.5);
        this.texts.xlabel.position.set(this.x + this.width/2, this.y + this.height-8);
        content.addChild(this.texts.xlabel);

        this.texts.xlabel_min = new PIXI.Text("0", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "center",
        });
        this.texts.xlabel_min.anchor.set(0.5,0.5);
        this.texts.xlabel_min.position.set(this.x + 50, this.y + this.height-22);
        content.addChild(this.texts.xlabel_min);

        this.texts.xlabel_mid = new PIXI.Text("0", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "center",
        });
        this.texts.xlabel_mid.anchor.set(0.5,0.5);
        this.texts.xlabel_mid.position.set(this.x + this.width/2, this.y + this.height-22);
        content.addChild(this.texts.xlabel_mid);

        this.texts.xlabel_max = new PIXI.Text("0", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "center",
        });
        this.texts.xlabel_max.anchor.set(0.5,0.5);
        this.texts.xlabel_max.position.set(this.x + this.width-30, this.y + this.height-22);
        content.addChild(this.texts.xlabel_max);

        // Y Labels.
        this.texts.ylabel = new PIXI.Text("Score", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "center",
        });
        this.texts.ylabel.angle = -90;
        this.texts.ylabel.anchor.set(0.5,0.5);
        this.texts.ylabel.position.set(this.x + 8, this.y + this.height/2);
        content.addChild(this.texts.ylabel);

        this.texts.ylabel_min = new PIXI.Text("0", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "right",
        });
        this.texts.ylabel_min.anchor.set(1,0.5);
        this.texts.ylabel_min.position.set(this.x + 48, this.y + this.height-30);
        content.addChild(this.texts.ylabel_min);

        this.texts.ylabel_mid = new PIXI.Text("0.5", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "right",
        });
        this.texts.ylabel_mid.anchor.set(1,0.5);
        this.texts.ylabel_mid.position.set(this.x + 48, this.y + this.height/2);
        content.addChild(this.texts.ylabel_mid);

        this.texts.ylabel_max = new PIXI.Text("1.0", {
            fontFamily: "Verdana", fontSize: 12, fill: 0xffffff, align: "right",
        });
        this.texts.ylabel_max.anchor.set(1,0.5);
        this.texts.ylabel_max.position.set(this.x + 48, this.y + 30);
        content.addChild(this.texts.ylabel_max);
    }
    ...
    Draw() {
        ...
        // Axes.
        graphics2.lineStyle(2, 0xffffff);
        graphics2.moveTo(this.x+50, this.y+30);
        graphics2.lineTo(this.x+50, this.y+this.height-30);
        graphics2.lineTo(this.x+this.width-30, this.y+this.height-30);

        // Gridlines.
        graphics2.lineStyle(2, 0x222222);
        graphics2.moveTo(this.x+this.width/2, this.y+30);
        graphics2.lineTo(this.x+this.width/2, this.y+this.height-31);
        graphics2.moveTo(this.x+51, this.y+this.height/2);
        graphics2.lineTo(this.x+this.width-30, this.y+this.height/2);

        // Plot data line graph.
        let nScores = scores_over_gen.length;
        let x_spacing = (this.width-80)/(nScores-1);
        let max = Math.max.apply(Math, scores_over_gen);
        this.texts.ylabel_max.text = ""+Sigs(max,0);
        this.texts.ylabel_mid.text = ""+Sigs(max/2,0);
        this.texts.xlabel_mid.text = ""+Sigs((nScores-1)/2,1);
        this.texts.xlabel_max.text = ""+nScores-1;
        let blcx = this.x+50;
        let blcy = this.y+this.height-30;
        let ph = this.height-60;
        graphics2.lineStyle(1,0xff0000);
        graphics2.moveTo(blcx, blcy);
        for (let i = 0; i < nScores; i++) {
            graphics2.lineTo(blcx + (i*x_spacing), blcy - ph*(scores_over_gen[i]/max));
        }
    }
}
...
function Init(){
    for (let i = 0; i < N_GAMES; i++) {
        let id = "game"+i;
        objs[id] = new Game(""+i, (i%N_GAMES_WIDE) * GAME_WIDTH, Math.floor((i/N_GAMES_WIDE)) * GAME_HEIGHT);
    }

    objs["scoreovergenplot"] = new ScoreOverGenPlot(20, GAME_HEIGHT*3 + 20, GAME_WIDTH-40, GAME_HEIGHT-40);
    objs["scoreovergenplot"].Draw();
}

function CheckGenerations() {
    let all_inactive = true;
    for (let i = 0; i < N_GAMES; i++) {
        if (objs["game"+i].active) {
            all_inactive = false;
            break;
        }
    }

    if (all_inactive && all_dead == false) {
        ...
        setTimeout(function () {
            let scores = [];
            for (let i = 0; i < N_GAMES; i++) {
                // NOTE: the "Math.random()" addition resolves ties randomly.
                scores.push(objs["game"+i].score + Math.random());
            }

            let max = Max(scores);
            scores_over_gen.push(max);

            // Determine the leading NN (first occurance).
            let leader = {};
            for (let i = 0; i < N_GAMES; i++) {
                if (scores[i] >= max) {
            ...
            // TODO: Do not just keep top and mutate the rest from top.
            //   Use the top few to create the next population.

            // Reset each game with the target parent.
            for (let i = 0; i < N_GAMES; i++) {
                objs["game"+i].Reset(leader);
            }

            all_dead = false;

            // Plots are drawn on graphics2, only clear on updates.
            graphics2.clear();
            objs["scoreovergenplot"].Draw();
        }, wait_time);
    }
